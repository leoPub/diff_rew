...
from modules.TPE.TNet import Encoder, Decoder, DecoderTau, VAEModel
...
class DR_QMIX:
    ...
    def train_predict(self, batch: EpisodeBatch, t_env: int):
        ...
        for epoch in range(self.iteration):
            ...
            full_loss = self.VAEModel.update(tau[random_agent][epoch],
                                             z_i, z_max, z_min,
                                             topology[random_batch[epoch],
                                             random_time[epoch]],
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, show_demo=False, save_data=None):
        ...
        with th.no_grad():
            ...
            topology_repeat = topology.clone().detach().unsqueeze(2).repeat(1, 1, 20, 1)
            ...
            for agent in range(self.args.n_agents):
                ...
                log_q_o = self.VAEModel.get_log_pi(f_prime_z, topology_repeat)

                log_p_i_o = self.VAEModel.get_log_pi(f_prime_i, topology_repeat)
                log_p_max_o = self.VAEModel.get_log_pi(f_prime_max, topology_repeat)
                log_p_min_o = self.VAEModel.get_log_pi(f_prime_min, topology_repeat)
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("total_reward",
                                 th.mean(rewards + diversity_reward + self.args.beta1 * normalized_visit), t_env)
